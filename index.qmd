...
title-slide-attributes:
  data-kicker: "IAEA 2026"    # small mono kicker above the title
...
## Traditional psychometrics {.hex-quiet}
...
:::{.columns}
:::{.column}
* DCMs place individuals into skill profiles
  * Fine-grained feedback to inform instructional decisions ([Thompson & Clark, 2024](https://doi.org/10.1111/emip.12619))
  * Group students with similar profiles
  * Answer more detailed questions about student achievement
...
tibble::tribble(
  ...
  mutate(
    across(
      ...
      \(x) {
        case_when(
          x == "xmark" ~ palette_measr[2],
          x == "check" ~ palette_measr[4]
        )
      },
      .names = "{.col}_color"
    )
  ...
  cols_label(skill1 = "Skill 1", skill2 = "Skill 2", skill3 = "Skill 3") |>
  fmt_icon(skill1, fill_color = from_column("skill1_color"), height = "50px") |>
  fmt_icon(skill2, fill_color = from_column("skill2_color"), height = "50px") |>
  fmt_icon(skill3, fill_color = from_column("skill3_color"), height = "50px") |>
...
## {.empty-navy data-menu-title="measr" background-color="#023047" background-iframe="grid-worms/index.html"}
...
ecpe_profiles |>
  ...
  opt_css(
    css = "
      #sticky_table .gt_col_heading {
        position: sticky; top: -2px; z-index: 10;
        box-shadow: inset 0 -3px 0 #023047;
      }
    "
  )
```
...
* Rather than individual attribute classifications, we can evaluate across the population of respondents ([Johnson & Sinharay, 2018](https://doi.org/10.1111/jedm.12196))
  * Accuracy: Probability the estimated classification is "correct"
  * Consistency: Probability that two parallel forms result in the same classification (i.e., test-retest)
...
* We can also evaluate the mastery probabilities across respondents ([Johnson & Sinharay, 2020](https://doi.org/10.3102/1076998619864550))
  * Informational: The proportion of uncertainty in an examinees mastery status that is eliminated by observing their item responses
...
## Evaluating scoring pipelines

* Scoring algorithms are often a "black box"

* measr is completely open source
  * Available on GitHub: <https://github.com/r-dcm/measr>
  * Examine exactly how measr estimates models and calculates evaluation metrics (e.g., model fit, reliability)

* Users can also host their analysis scripts using measr, allowing other experts or the public to evaluate how the software was used
...
:::{.column width="50%"}
[![](figure/rupp-book.jpg){fig-alt="Cover of Diagnostic Measurement book by Rupp, Templin, and Henson." height="18em"}](https://www.amazon.com/Diagnostic-Measurement-Applications-Methodology-Sciences/dp/1606235273)
:::

:::{.column width="50%"}
[![](figure/dcm-handbook.jpeg){fig-alt="Cover of the Handbook of Diagnostic Classification Models by von Davier and Lee." height="18em"}](https://link.springer.com/book/10.1007/978-3-030-05584-4)
:::
...
:::{.larger .spaced}
{{< fa globe >}} \ [Case studies to get started](https://r-dcm.org/start)
